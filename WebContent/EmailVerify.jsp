...
<html>
	<head>
		<title>Safe Banking Email Verify</title>
		...
		<link type="text/css" rel="stylesheet" href="CSS/EmailVerify.css">
		
		<script type="text/javascript" src="JS/Jquery.js"></script>
		<script type="text/javascript" src="JS/Tooltip.js"></script>
		<script type="text/javascript" src="JS/EmailVerify.js"></script>
		<script type="text/javascript" src="JS/EmailVerifyLog.js"></script>
	</head>
	<body style="background-image: url('img/2.jpg');">
		...
		<%!
			...
			String acc = "nullnullnullnullnull";
			String ex = "null";
		%>
		<%-- <%
			System.out.println("\nFinally in EmailVerify.jsp\n");
			show.ReqParam(request, response);
			show.SessionParam(request, response);
			if(session.getAttribute("emailVerificationComplete")!=null){
			%>
				<script type="text/javascript">
					$(document).ready(function(){
						console.log("reached in emailVerify.jsp. Now going to call emailVerificationComplete function from script tag");
						emailVerificationComplete();
					});
				</script>
			<%
			}
			else if(session.getAttribute("verify")==null){
				session.invalidate();
				response.sendRedirect("Welcome.jsp");
				return;
			}
			else{
				BankDetails up=(BankDetails)session.getAttribute("verify");
				fullname=up.getFname()+" "+up.getLname();
				acc = up.getAcc();
				uname = up.getUname();
				ex=up.getEmail();
				%>
				<script type="text/javascript">
					$(document).ready(function(){
						console.log("reached in emailVerify.jsp. Now going to call emailVerificationInitiated function from script tag");
		...
		<div class="two">
			<div class="twoc1">
				<h2 class="twoc1c1" id="twoc1p1">Registration Successful</h2>
				<h4 class="twoc1c1" id="twoc1p2">An Email has been sent to the email-id&nbsp;&nbsp;&nbsp;<span id="email"><%=ex %></span>. The email contains your account number and verification password.</h4>
				<h6>&nbsp;</h6> 
				<h5 class="twoc1c1" id="twoc1p3">Enter the password below and then set a new password to activate your account.</h5>
			</div>
			<div class="twoc2">
				<h3 class="twoc1c1" id="twoc2p1">Password Matched</h3>
				<h4 class="twoc1c1" id="twoc2p2">Your Email will be verified only after you set a new password and choose a security QnA.</h4>
				...
				<form action="EmailVerify" method="post">
					<table id="twoc3p1">
					    <tr>
						    ...
						    <td width="50%"><input type="text" name="fullname" size="40" value="<%= fullname %>"readonly/></td>
					    </tr>
					    <tr>
						    <td>Your Account Number: </td>
						    <td><input type="text" name="acc" size="40" value="<%= acc.substring(0, 2)+"XXXXXXXXXX"+acc.substring(12) %>" readonly/></td>
					    </tr>
					</table>
					<table id="twoc3p2">
					    <tr>
					...
					<input type="hidden" name="emailverifypagetime" value="0"/>
		...
		<div class="four">
			<div id="fourp1">
				<h2 id="msgp1">Registration Successful.<br> Please verify your email-id here to activate your account.</h2>
				<img id="fourp1p1" alt="next" src="gifs/next.gif">
			</div>
			<div id="fourp2">
				<h2>Your Email id has been verified.<br> Please change your password in the next section.</h2>
				<img id="fourp2p1" alt="next" src="gifs/next.gif">
			</div>
			<div id="fourp3">
				<img id="fourp3p1" alt="incorrect" src="gifs/wrong.gif">
				<h2>Incorrect Password. Please Try again.</h2>
			</div>
			<div id="fourp4">
				<img id="fourp4p1" alt="complete" src="gifs/source1.gif">
				<h2>Your Email is verified and the password is changed successfully.<br> <br>Redirecting to Home Page in 5s.</h2>
			</div>
		</div>
