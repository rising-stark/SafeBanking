...
@WebServlet(asyncSupported = true, urlPatterns = { "/EmailVerify" })
public class EmailVerify extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			BankDetails bank = (BankDetails) session.getAttribute("verify");
			bankDAO u = new bankDAO();
			GetTime obj = new GetTime();
			String regid="null", uname="null", acc="null", action="null";	
			...
			if(session.getAttribute("emailVerificationComplete")!=null && action != null && action.equals("emailVerificationComplete")){
				...
				AddRegisteringLogList.add(regid, uname, "EmailVerify.java", "Email Verify Completed. Password Changed Successfully. Email Sent to "+email, request, response);
			...
			if(session.getAttribute("verify")!=null && action != null && action.equals("emailVerificationInitiated")){
				response.setContentType("text/plain");
		    	response.setCharacterEncoding("UTF-8");
				...
				System.out.println("Time before sending email "+obj.now());
				AddRegisteringLogList.addTime(regid, uname, time, "EmailVerify.java", "Email Verify Initiated. Sending Email to "+email, request, response);
				WorkingEmail.sendEmail(email, subject, body);
				System.out.println("Time after sending email "+obj.now());
				AddRegisteringLogList.add(regid, uname, "EmailVerify.java", "Email Verify Initiated. Email Sent to "+email, request, response);
			...
			if (action != null && action.equals("verifyEmail")) {
				...
				AddRegisteringLogList.addTime(regid, uname, verifytime, "EmailVerify.jsp", message+" "+oldpass,request,response);
			...
			if(action!=null && action.equals("setEmailverifypagetime")) {
				...
				AddRegisteringLogList.addTime(regid, uname, time, "EmailVerify.jsp", "Successfully Loaded",request,response);
				return;
			}
			...
			AddRegisteringLogList.addTime(regid, uname, subtime, "EmailVerify.java", "Processing Information",request,response);
			...
			bank.setBankverified(regid);
			bank.setBankmodified(regid);
			...
			AddRegisteringLogList.add(regid, uname, "EmailVerify.java", message, request, response);
			
			u.updateVerifyEmail(acc, regid, message);
